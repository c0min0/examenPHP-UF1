...
/**
 * Obté el hash SHA256 del password d'un usuari
 *
 * @param string $email email de l'usuari a consultar
 * 
...
//Ex5
/**
 * Obté el rol de l'usuari, donat un ID
 *
 * @param int $userId ID de l'usuari a consultar
 * 
 * @return string role de l'usuari
 */
function getUserRoleById($userId)
{
    try {
        ...
        $statement = $connexio->prepare('SELECT role FROM users WHERE id = :userId');
...
//Ex5
/**
 * Obté el nickname de tots els usuaris exepte l'usuari especificat
 * 
 * @return array ID de l'usuari
 */
function getUserNicknamesExept($userId)
{
    try {
        $connexio = getConnection();
 ...
 * @param string $hash256 hash SHA256 del password del nou usuari
 * 
 */
function insertNewUser($email, $nickname, $hash256)
{
    try {
        $connexio = getConnection();
...
//Ex6 Aquí caldria especificar un camp a la taula que fos usuari_actiu o quelcom semblant,
// però no m'ha donat temps a implementar-ho
